...
use Molongui\Contributors\Common\Modules\Settings\Settings_Page;
...
use Molongui\Contributors\Common\Utils\User;
...
class Settings
{
    ...
    public function __construct()
    {
        ...
        add_filter( 'molongui_contributors/options_script_params', array( __CLASS__, 'localize_scripts' ), 10 );
        add_action( 'admin_enqueue_scripts', array( __CLASS__, 'register_styles' ) );
        add_filter( 'molongui_contributors/options_extra_styles', array( __CLASS__, 'extra_styles' ) );
        add_action( 'molongui_contributors/options', array( __CLASS__, 'add_defaults' ) );
        add_filter( 'molongui_contributors/sanitize_option', array( $this, 'custom_snippets_input' ), 10, 3 );
        add_action( 'wp_ajax_'.MOLONGUI_CONTRIBUTORS_PREFIX.'_save_options', array( $this, 'save' ) );
        add_action( 'wp_ajax_'.MOLONGUI_CONTRIBUTORS_PREFIX.'_export_options', array( $this, 'export' ) );
    ...
    {
        $settings = (array) get_option( MOLONGUI_CONTRIBUTORS_PREFIX.'_options', array() );

        if ( empty( $settings ) )
        {
            $settings = self::get_defaults();
        }
        $settings = apply_filters( 'molongui_contributors/get_options', $settings );
    ...
    public static function get_defaults()
    {
        ...
        return apply_filters( 'molongui_contributors/default_options', $fw_options );
    }
    public static function add_defaults()
    {
        $options  = self::get();
        $defaults = self::get_defaults();
        update_option( MOLONGUI_CONTRIBUTORS_PREFIX.'_options', array_merge( $defaults, $options ), true );
    }
    public static function get_config()
    {
        ...
        (
            $wpdb->prepare( "SELECT option_name,option_value FROM {$wpdb->options} WHERE option_name LIKE %s", MOLONGUI_CONTRIBUTORS_PREFIX.'_%' ),
            ARRAY_A
        );
    ...
    public function save()
    {
        ...
        {
            $options[$key] = apply_filters( 'molongui_contributors/sanitize_option', sanitize_text_field( $value ), $key, $value );
        }

        if ( isset( $options ) and is_array( $options ) )
        {
            $options['plugin_version'] = MOLONGUI_CONTRIBUTORS_VERSION;
            $current = (array) get_option( MOLONGUI_CONTRIBUTORS_PREFIX.'_options', array() );
            $options = array_merge( $current, $options );
            $options = apply_filters( 'molongui_contributors/validate_options', $options, $current );
            update_option( MOLONGUI_CONTRIBUTORS_PREFIX.'_options', $options, true );

            $old = $current;
            do_action( 'molongui_contributors/options', $options, $old );
        }
        wp_die();
    }
    public function export()
    {
        ...
        $options['plugin_id']      = MOLONGUI_CONTRIBUTORS_PREFIX;
        $options['plugin_version'] = MOLONGUI_CONTRIBUTORS_VERSION;
        $options = apply_filters( 'molongui_contributors/export_options', $options );
    ...
    public function send_mail()
    {
        ...
        switch( $_POST['type'] )
        {
            case 'ticket':
                ...
                $subject = sprintf( esc_html__( "Support Ticket %s: %s", 'molongui-post-contributors' ), sanitize_text_field( $params['ticket-id'] ), sanitize_text_field( $params['your-subject'] ) );
                ...
                $message .= '<small>'.sprintf( esc_html__( "This support ticket was sent using the form on the Support Page (%s)", 'molongui-post-contributors' ), $_POST['domain'] ).'</small>';
            ...
            case 'report':
                ...
                $subject = sprintf( esc_html__( "Support Report for %s", 'molongui-post-contributors' ), sanitize_text_field( $_POST['domain'] ) );
                $message = '';
                $headers = array
                (
    ...
    public static function get_mail_appendix()
    {
        ...
        $appendix .= '<p style="'.$css_title.'">'.__( "Molongui Plugins", 'molongui-post-contributors' ).'</p>';
        ...
        $appendix .= '<p style="'.$css_title.'">'.__( "Client Browser", 'molongui-post-contributors' ).'</p>';
        $appendix .= '<p style="'.$css_item.'">'.$client['browser'].' on '.$client['platform'].'</p>';
        $appendix .= '<p style="'.$css_title.'">'.__( "Current User", 'molongui-post-contributors' ).'</p>';
        $appendix .= '<p style="'.$css_item.'">'.$current_user->display_name.' with registered e-mail '.$current_user->user_email.'</p>';
        $appendix .= '<p style="'.$css_title.'">'.__( "System Report", 'molongui-post-contributors' ).'</p>';
        $appendix .= '<p style="'.$css_item.'">'.nl2br( Debug::get_debug_data() ).'</p>';
        $appendix .= '<p style="'.$css_title.'">'.__( "Plugin Settings", 'molongui-post-contributors' ).'</p>';
        foreach ( $molonguis as $plugin )
        {
    ...
    {
        $subject = __( "We got your email! Hang tight!", 'molongui-post-contributors' );
        /*! // translators: %1$s: User name. %2$s: Line breaks. */
        $message = sprintf( esc_html__( "Hi %s! %s This is an automatic email just to let you know we've got your help request . We'll get you an answer back shortly.", 'molongui-post-contributors' ), $user['name'], '<br><br>' );
        $headers = array
        (
    ...
    {
        return apply_filters( 'molongui_contributors/custom_css', self::get( 'custom_css' ) );
    }
    public static function get_custom_php()
    {
        return apply_filters( 'molongui_contributors/custom_php', self::get( 'custom_php' ) );
    }
    public function load_custom_snippets()
    {
        if ( Request::get( 'nophpContributors' ) )
        {
            return;
        }
        $load_on_admin = ( apply_filters( 'molongui_contributors/enable_custom_php_in_admin', false ) or Request::get( 'phpContributors' ) or Settings::get( 'enable_custom_php_in_admin' ) );
