...
class Assets
{
    public static function register_script( $file, $scope, $deps = array( 'jquery' ), $handle = null, $version = null )
    {
        ...
        {
            do_action( "molongui_contributors/{$scope}/pre_register_script", $scope );

            $handle  = !empty( $handle )  ? $handle  : str_replace( '_', '-', MOLONGUI_CONTRIBUTORS_PREFIX . '-' . $scope );
            $version = !empty( $version ) ? $version : MOLONGUI_CONTRIBUTORS_VERSION;

            wp_register_script( $handle, plugins_url( '/' ).$file, $deps, $version, true );
            $params = apply_filters( "molongui_contributors/{$scope}_script_params", '' );
            ...
            {
                $function = 'molongui_contributors_'.$scope.'_script_params';
            ...
            do_action( "molongui_contributors/{$scope}/script_registered", $scope );
        }
    }
    public static function enqueue_script( $file, $scope, $admin = false, $handle = null, $version = null )
    {
        ...
        if ( file_exists( $filepath ) )
        {
            ...
            $handle  = !empty( $handle )  ? $handle  : str_replace( '_', '-', MOLONGUI_CONTRIBUTORS_PREFIX . '-' . $scope );
            $version = !empty( $version ) ? $version : MOLONGUI_CONTRIBUTORS_VERSION;
            $inline = apply_filters( "molongui_contributors/{$scope}/inline_script", $filesize < 4096 );
            do_action( "molongui_contributors/{$scope}/pre_enqueue_script", $scope, $inline );
            if ( $inline )
            {
                /*! This action is documented in includes/helpers/assets/scripts.php */
                if ( !did_action( "_molongui_contributors/{$scope}/script_inlined" ) )
                {
                    ...
                    {
                        do_action( "molongui_contributors/{$scope}/pre_inline_script", $scope, $filepath, $handle );
                     ...
                     */
                    do_action( "_molongui_contributors/{$scope}/script_inlined" );
                }
            }
            else
            {
            ...
            do_action( "molongui_contributors/{$scope}/script_loaded", $scope );
        }
    }
    public static function register_style( $file, $scope, $deps = array(), $handle = null, $version = null )
    {
        ...
        if ( file_exists( trailingslashit( WP_PLUGIN_DIR ) . $file ) )
        {
            do_action( "molongui_contributors/{$scope}/pre_register_styles", $scope );

            $handle  = !empty( $handle )  ? $handle  : str_replace( '_', '-', MOLONGUI_CONTRIBUTORS_PREFIX . '-' . $scope );
            $version = !empty( $version ) ? $version : MOLONGUI_CONTRIBUTORS_VERSION;

            wp_register_style( $handle, plugins_url( '/' ) . $file, $deps, $version, 'all' );
            do_action( "molongui_contributors/{$scope}/styles_registered", $scope );
        }
    }
    public static function enqueue_style( $file, $scope, $admin = false, $handle = null, $version = null )
    {
        ...
        if ( file_exists( $filepath ) )
        {
            ...
            $handle  = !empty( $handle )  ? $handle  : str_replace( '_', '-', MOLONGUI_CONTRIBUTORS_PREFIX . '-' . $scope );
            $version = !empty( $version ) ? $version : MOLONGUI_CONTRIBUTORS_VERSION;
            $inline = apply_filters( "molongui_contributors/{$scope}/inline_styles", $filesize < 4096 );
            do_action( "molongui_contributors/{$scope}/pre_enqueue_styles", $scope, $inline );
            if ( $inline )
            {
                /*! This action is documented in includes/helpers/assets/styles.php */
                if ( !did_action( "_molongui_contributors/{$scope}/styles_inlined" ) )
                {
                    $hook = is_admin() ? 'admin_print_footer_scripts' : 'wp_print_footer_scripts';
                    ...
                    add_action( $hook, function() use ( $scope, $filepath, $handle, $version )
                    {
                         ...
                         */
                        $contents = apply_filters( "_molongui_contributors/{$scope}/styles_contents", $contents, $filepath );
                        $extra = apply_filters( "molongui_contributors/{$scope}_extra_styles", '' );
                        if ( empty( $extra ) )
                        {
                            $function = 'molongui_contributors_'.$scope.'_extra_styles';
                            if ( function_exists( $function ) )
                            {
                     ...
                     * risk and knowing that it could cause code failure.
                     */
                    do_action( "_molongui_contributors/{$scope}/styles_inlined" );
                }
            }
            else
            {
            ...
            do_action( "molongui_contributors/{$scope}/styles_loaded", $scope );
        }
    }
    public static function register_media_uploader()
    {
        $file = MOLONGUI_CONTRIBUTORS_FOLDER . ( is_rtl() ? '/assets/css/common/media-upload-rtl.min.css' : '/assets/css/common/media-upload.min.css' );
        $deps = array();
    ...
    public static function register_sweetalert()
    {
        ...
        if ( apply_filters( 'molongui_contributors/assets/load_remote', true ) )
        {
            $sweetalert_js_url = 'https://cdn.jsdelivr.net/npm/sweetalert@'.$version.'/dist/sweetalert.min.js';
        }
        else
        {
            $sweetalert_js_url = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/sweetalert/sweetalert.min.js';
        }
    ...
    public static function register_selectr()
    {
        ...
        {
            $selectr_js_url  = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/selectr/selectr.min.js';
            $selectr_css_url = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/selectr/selectr.min.css';
        }
    ...
    public static function register_sortable()
    {
        ...
        {
            $sortable_js_url  = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/sortable/Sortable.min.js';
        }
    ...
    public static function register_element_queries()
    {
        ...
        if ( apply_filters( 'molongui_contributors/load_element_queries', true ) )
        {
            if ( apply_filters( 'molongui_contributors/assets/load_remote', true ) )
            {
            ...
            {
                $resizesensor_js_url   = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/element-queries/ResizeSensor.min.js';
                $elementqueries_js_url = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/element-queries/ElementQueries.min.js';
            }
    ...
    public static function register_semantic_ui_dropdown()
    {
        ...
        {
            $dropdown_js_url  = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/semantic/dropdown.'.$version.'.min.js';
            $dropdown_css_url = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/semantic/dropdown.'.$version.'.min.css';
        }
    ...
    public static function register_semantic_ui_transition()
    {
        ...
        {
            $transition_js_url  = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/semantic/transition.'.$version.'.min.js';
            $transition_css_url = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/semantic/transition.'.$version.'.min.css';
        }
    ...
    public static function register_semantic_ui_icon()
    {
        ...
        {
            $icon_css_url = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/semantic/icon.'.$version.'.min.css';
        }
    ...
    public static function register_semantic_ui_label()
    {
        ...
        {
            $label_css_url = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/semantic/label.'.$version.'.min.css';
        }
    ...
    public static function register_semantic_ui_popup()
    {
        ...
        {
            $popup_js_url  = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/semantic/popup.'.$version.'.min.js';
            $popup_css_url = MOLONGUI_CONTRIBUTORS_URL . 'common/assets/vendor/semantic/popup.'.$version.'.min.css';
        }
