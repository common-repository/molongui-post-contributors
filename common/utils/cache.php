...
class Cache
{
    public static function get( $key )
    {
        ...
        return wp_cache_get( $key, MOLONGUI_CONTRIBUTORS_NAME );
    }
    public static function set( $key, $data )
    {
        if ( empty( $key ) or empty( $data ) ) return false;
        return wp_cache_set( $key, $data, MOLONGUI_CONTRIBUTORS_NAME );
    }
    public static function clear( $key )
    {
        ...
        $hashes = get_option( MOLONGUI_CONTRIBUTORS_PREFIX . '_cache_' . $key, array() );
        delete_option( MOLONGUI_CONTRIBUTORS_PREFIX . '_cache_' . $key );
        foreach ( $hashes as $hash ) wp_cache_delete( $key . '_' . $hash, MOLONGUI_CONTRIBUTORS_NAME );
    }
    public static function query( $args, $object )
    {
        $cache = apply_filters( 'molongui_contributors/cache', true );
        if ( !$cache )
        {
        ...
        else
        {
            ...
            if ( false === $data )
            {
                ...
                self::set( $key, $data );
                $db_key = MOLONGUI_CONTRIBUTORS_PREFIX . '_cache_' . $object;
