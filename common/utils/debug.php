...
class Debug
{
    ...
    {
        return apply_filters( 'molongui_contributors/debug', defined( 'MOLONGUI_CONTRIBUTORS_DEBUG' ) and MOLONGUI_CONTRIBUTORS_DEBUG );
    }
    public static function console_log( $value = null, $message = '' )
    {
        if ( self::is_enabled() )
        {
            if ( apply_filters( 'molongui_contributors/disable_console_log', false ) )
            {
                return;
            }
            ...
            add_action( $hook, function() use ( $value, $message )
            {
                ...
                if ( defined( 'MOLONGUI_CONTRIBUTORS_TITLE' ) )
                {
                    $intro = '"' . '%c' . strtoupper( MOLONGUI_CONTRIBUTORS_TITLE ) . '\n%c' . $message . '", "background:yellow; color: black; font-weight: bold; text-decoration: underline;", ""';
                }
                else
                {
                    $intro = '"' . $message . '"';
    ...
    {
        if ( apply_filters( 'molongui_contributors/disable_dump', false ) )
        {
            return;
        }
        ...
        if ( is_admin() )
        {
            ...
            add_filter( '_molongui_contributors/force_inline_dump', function( $value )
            {
            ...
            $force_inline = apply_filters( '_molongui_contributors/force_inline_dump', false );
            if ( !did_action( 'admin_notices' ) and !$die and !$force_inline )
            {
                add_action( 'admin_notices', function() use ( $debug )
                {
                    $html_message = '';
                    if ( Helpers::is_block_editor() and !did_action( '_molongui_contributors/hide_block_editor' ) )
                    {
                        do_action( '_molongui_contributors/hide_block_editor' );

                        echo '<style>#editor{display:none} .wrap.hide-if-js.block-editor-no-js{display:block}</style>';
                        $html_message .= sprintf( '<div class="notice notice-warning" style="display: block !important; background: #dba6171f;"><h2>Hidden Block Editor</h2><pre>%s</pre></div>', esc_html__( 'The Block Editor has been hidden so debug information can be seen.', 'molongui-post-contributors' ) );
                    }
