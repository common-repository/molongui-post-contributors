...
class Plugin
{
    ...
    public static function custom_admin_footer( $footer_text )
    {
        ...
        if ( $current_screen->id == 'molongui_page_' . MOLONGUI_CONTRIBUTORS_NAME )
        {
            /*! // translators: %1$s: Plugin name. %2$s: Opening a tag. %3$s: Closing a tag. */
            return ( sprintf( esc_html__( "If you like <strong>%s</strong> please leave us a %s&#9733;&#9733;&#9733;&#9733;&#9733;%s rating. A huge thank you in advance!", 'molongui-post-contributors' ),
                MOLONGUI_CONTRIBUTORS_TITLE,
                '<a href="https://wordpress.org/support/view/plugin-reviews/' . MOLONGUI_CONTRIBUTORS_NAME . '?filter=5#postform" target="_blank" class="molongui-admin-footer-link" data-rated="' . esc_attr__( "Thanks :)", 'molongui-post-contributors' ) . '">',
                '</a>' )
            );
        }
    ...
    {
        return did_action( 'molongui_contributors_pro/init' );
    }
    public static function add_go_pro_link( $links )
    {
        $more_links = array
        (
            'settings' => '<a href="' . admin_url( 'admin.php?page=' . MOLONGUI_CONTRIBUTORS_NAME ) . '">' . __( "Settings" ) . '</a>',
            'docs'     => '<a href="' . 'https://www.molongui.com/help/docs/' . '" target="blank" >' . __( "Docs", 'molongui-post-contributors' ) . '</a>'
        );

        if ( apply_filters( 'molongui_contributors/action_links/go_pro', true ) )
        {
            $more_links['gopro'] = '<a href="' . MOLONGUI_CONTRIBUTORS_WEB . '/" target="blank" style="font-weight:bold;color:orange">' . __( "Go Pro", 'molongui-post-contributors' ) . '</a>';
        }
